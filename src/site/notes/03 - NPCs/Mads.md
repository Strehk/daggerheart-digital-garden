---
{"dg-publish":true,"permalink":"/03-np-cs/mads/","tags":["npc","kalthavn","mads"],"dg-note-properties":{"type":"npc","role":"unknown","ancestry":null,"community":null,"location":"[[Nordhavn]]","affiliations":[],"status":"unknown","first_seen":"[[Session 01 - Morgenlicht über Kalthavn]]","tags":["npc","kalthavn","mads"]}}
---


# Mads

> Ein **Junge (~12–13 Jahre)** mit **blondem, krausem Haar**. Machte sich am Hafen an einer **Kiste der [[04 - Welt/Fraktionen/Streicher\|Streicher]]** zu schaffen — und entkam uns am Ende auf magische Weise.

## Erscheinung
- Jung, etwa 12–13 Jahre alt.
- Blondes, krauses Haar.

## Auftreten & Verhalten
- Schnappte sich aus der Streicher-Kiste einen **Stoffbeutel** und floh damit in ein **Fischerboot** — wir bekamen ihn erst durch **Teamwork** zu fassen (Verfolgungsjagd, DC 14; siehe [[05 - Sessions/Session 02 - Die krumme Laterne\|Session 02 - Die krumme Laterne]]).
- Sagte, er würde **„das Richtige"** tun.
- Als die [[04 - Welt/Fraktionen/Eisige Faust\|Eisige Faust]] ([[03 - NPCs/Hauptmann Sven Svenson\|Hauptmann Sven Svenson]]) [[02 - Party/Raksha\|Raksha]] an der Schulter packte, **verschwand Mads mithilfe einer grünen Runen-Scheibe** spurlos. (#offene-frage)

## Was wir wissen
- Heißt **Mads**.
- Hatte den Beutel mit den **[[07 - Items/Bernstein-Laterne\|Bernstein-Laternen]]** aus der Streicher-Kiste an sich genommen — den wir jetzt haben.
- Verfügt über (oder hatte) ein **magisches Fluchtmittel**: die zerbrochene grüne Rune-Scheibe (Teleport?).

## Beziehungen
- **Verbündete / Bekannte:** unklar — wer hat ihn auf die Kiste angesetzt?
- **Feinde / Gegner:** 
- **Fraktionen:** Bezug zu den [[04 - Welt/Fraktionen/Streicher\|Streicher]]n bzw. ihrer Schmuggel-Ladung unklar.

## Bisherige Begegnungen
*Backlinks zu Sessions erscheinen automatisch unten.*

## Vermutungen / Theorien
> [!note] Vermutung
> - Wer hat einem Kind eine **magische Teleport-Scheibe** in die Hand gegeben — und ihn auf genau diese Laternen-Kiste angesetzt?
> - Mads glaubt, **„das Richtige"** zu tun — wurde er überzeugt/instrumentalisiert, oder verfolgt er ein eigenes Ziel mit den Laternen?
> - Arbeitet Mads für den **Kontakt im schwarzen Mantel** (siehe [[03 - NPCs/Kapitän Arrik\|Kapitän Arrik]] / [[04 - Welt/Fraktionen/Streicher\|Streicher]]), für die [[04 - Welt/Fraktionen/Tuchmacher\|Tuchmacher]] oder für sich selbst?

## Notizen
- 
